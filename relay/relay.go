...
func NewClient(localURL *url.URL, opts *ClientOptions) *Client {
	...
	return &Client{
		...
		sendChan: make(chan *OutgoingMessage),
...
func (c *Client) connect(ctx context.Context) error {
	var err error
	conn, _, err := c.dialer.DialContext(ctx, c.websocketURL, nil)
	if err != nil {
		return err
	}
	c.conn = conn
...
func (c *Client) makeLocalRequest(url *url.URL, msg IncomingMessageEventData) (*http.Response, error) {
	decBody, err := base64.StdEncoding.DecodeString(msg.Body)
	if err != nil {
		return nil, err
	}
	...
	req := &http.Request{
		...
		Header: http.Header{},
		URL:    url,
		Body:   io.NopCloser(bytes.NewReader(decBody)),
	}
